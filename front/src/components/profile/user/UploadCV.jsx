...
const UploadCV = () => {
      ...
      async () => {
        try {
          ...
          try {
            ...
            await axios.post(
              `${import.meta.env.VITE_BASE_URL}/cv/uploadCv`,
  ...
  const fetchUserCVs = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/cv/getUserCvs`,
        // "http://localhost:8000/api/cv/getUserCvs",
        { withCredentials: true }
      );

      "Full response from backend:", response; // Log the full response
  ...
  const handleDeleteCV = async (cvId) => {
    try {
      await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/cv/deleteCv/${cvId}`,
        // `http://localhost:8000/api/cv/deleteCv/${cvId}`
        {
          withCredentials: true,
        }
      );
  ...
  const handleDownload = async (fileName) => {
    try {
      const response = await axios({
        url: `${import.meta.env.VITE_BASE_URL}/cv/downloadCv/${fileName}`,
        // `http://localhost:8000/api/cv/downloadCv/${fileName}`, // Use the backend route to download
        method: "GET",
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 p-6 bg-white rounded-lg shadow-lg">
      ...
      <h3 className="text-lg font-semibold mb-4">העלאת קורות חיים</h3>
            ...
            <table className="min-w-full border-collapse">
              <thead>
                <tr className="bg-gray-200 text-center">
                  <th className="py-2 px-4 border">שם קובץ</th>
                  <th className="py-2 px-4 border">תאריך העלאה</th>
                  <th className="py-2 px-4 border">פעולות</th>
                </tr>
              </thead>
              <tbody>
                {cvList.map((cv, index) => (
                  <tr key={cv._id} className="hover:bg-gray-100 text-center">
