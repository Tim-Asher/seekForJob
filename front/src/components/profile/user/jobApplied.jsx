...
const JobApplied = () => {
  ...
  const handleSubmitReview = async () => {
    try {
      ...
      // Reset review content to initial state
      setReviewContent({
        process: "1",
        responseTime: "1",
        attitude: "1",
      ...
      if (error?.status === 400) {
        errorToast("You can only submit one review per job.");
      } else {
  ...
  const handleViewRatings = (employerId) => {
    navigate(`/employerReviews/${employerId}`);
  };
  ...
  return (
    <div className="space-y-4 p-4">
      ...
      {jobs &&
        jobs.map((job) => (
          <div key={job._id} className="shadow rounded bg-white text-right">
            <button
              onClick={() => toggleJob(job._id)}
              className="w-full text-right p-4 text-lg font-bold"
            >
              {job.jobTitle}
            </button>
            {openJobId === job._id && (
