...
const ResumeBuilder = () => {
  const { isLogin, whosLogin, successToast, errorToast } =
  ...
  const handleFieldChange = (field, index) => (event) => {
    const newFields = data[field].map((item, i) =>
            ...
            ...(data.socialMedia
              ? [
                  new Paragraph({
                    children: [
                      new TextRun({
                        text: `socialMedia: ${data.socialMedia}`,
                      }),
                    ],
                  }),
                ]
            ...
            ...data.experience.map(
              (exp) =>
                new Paragraph({
                  children: [
                    new TextRun({
                      text: `משרה: ${exp.position}`,
                    }),
                    new TextRun({
                      break: 1, // Adds a new line
                    }),
                    new TextRun({
                      text: `חברה: ${exp.company}`,
                    }),
                    new TextRun({
                      break: 1, // Adds a new line
                    }),
                    new TextRun({
                      text: `${
                        exp.startDate
                          ? `תאריך התחלה: ${exp.startDate.toLocaleDateString()}`
                          : ""
                      }${
                        exp.endDate
                          ? `, תאריך סיום: ${exp.endDate.toLocaleDateString()}`
                          : ""
                      }${exp.current ? ", עדיין עובד כאן" : ""}`,
                    }),
                    new TextRun({
                      break: 1, // Adds a new line
                    }),
                    new TextRun({
                      text: `תיאור: ${exp.description}`,
                    }),
                  ],
                })
            ),
            ...
            ...data.education.map(
              (edu) =>
                new Paragraph({
                  children: [
                    new TextRun({
                      text: `תואר: ${edu.degree}`,
                    }),
                    new TextRun({
                      break: 1, // Adds a new line
                    }),
                    new TextRun({
                      text: `בית ספר: ${edu.school}`,
                    }),
                    new TextRun({
                      break: 1, // Adds a new line
                    }),
                    new TextRun({
                      text: `${
                        edu.startDate
                          ? `תאריך התחלה: ${edu.startDate.toLocaleDateString()}`
                          : ""
                      }${
                        edu.endDate
                          ? `, תאריך סיום: ${edu.endDate.toLocaleDateString()}`
                          : ""
                      }${edu.current ? ", סטודנט" : ""}`,
                    }),
                    new TextRun({
  ...
  const handleSave = () => {
    if (!isLogin) {
      errorToast("You must log in!");
    } else if (whosLogin === "employer") {
      errorToast("employer cannot upload cv!");
    } else {
    }
  };
  ...
  return (
    <div className="p-4 m-3 flex flex-col items-end rounded-xl max-w-screen-md mx-auto bg-gray-200 shadow-lg shadow-gray-500">
      <h1 className="text-2xl text-center font-bold mb-4 text-orange-500">
        יצירת קורות חיים
      </h1>
      ...
      {!validateEmail(data.email) && data.email && (
        <p className="text-red-500">כתובת דוא"ל לא תקינה</p>
      )}
      ...
      <div className="relative mb-4" style={{ direction: "ltr" }}>
        <PhoneInput
          ...
          inputClass="!text-left !pl-[50px] !pr-3"
      ...
      <input
        ...
        onChange={handleChange}
      />
      {!validateURL(data.linkedin) && data.linkedin && (
        <p className="text-red-500">קישור לא תקין</p>
      )}
      <input
        type="url"
        ...
        onChange={handleChange}
      />
      {!validateURL(data.socialMedia) && data.socialMedia && (
        <p className="text-red-500">קישור לא תקין</p>
      )}
      ...
      {data.education.map((edu, index) => (
        <div
          key={index}
          className="relative border flex flex-col items-end w-full border-gray-200 p-4 rounded-md mb-4"
        >
          ...
          <DatePicker
            ...
            onChange={(date) =>
              handleDateChange("education", index, "startDate", date)
            }
            className="w-full p-2 border border-gray-500 rounded mb-4"
          ...
          <DatePicker
            ...
            onChange={(date) =>
              handleDateChange("education", index, "endDate", date)
            }
            className="w-full p-2 border border-gray-500 rounded mb-4"
            dateFormat="dd/MM/yyyy"
      ...
      {data.experience.map((exp, index) => (
        <div
          key={index}
          className="relative  flex flex-col items-end w-full border border-gray-200 p-4 rounded-md mb-4"
        >
          ...
          <DatePicker
            ...
            onChange={(date) =>
              handleDateChange("experience", index, "startDate", date)
            }
            className="w-full p-2 border border-gray-500 rounded mb-4"
            dateFormat="dd/MM/yyyy"
          />
          ...
          <DatePicker
            ...
            onChange={(date) =>
              handleDateChange("experience", index, "endDate", date)
            }
            className="w-full p-2 border border-gray-500 rounded mb-4"
            dateFormat="dd/MM/yyyy"
      ...
      {data.projects.map((proj, index) => (
        ...
        >
          ...
          />
          {!validateURL(proj.link) && proj.link && (
            <p className="text-red-500">קישור לא תקין</p>
          )}
