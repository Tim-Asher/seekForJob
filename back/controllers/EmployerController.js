...
exports.addEmployerReview = asyncHandler(async (req, res, next) => {
  ...
  // Check if the user has already reviewed this employer
  const existingReview = employer.reviews.find(
    (review) => review.userId.toString() === req.user._id.toString()
  );
  if (existingReview) {
    return res.status(400).json({
      status: "fail",
      message: "You can only submit one review for this employer.",
    });
  }
